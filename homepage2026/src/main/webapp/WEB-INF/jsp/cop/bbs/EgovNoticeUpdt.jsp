                        ...
                        <table>
					      ...
					      <tr>
					        ...
					        <td colspan="3">
					          ...
					          <br/><form:errors path="nttCn" cssClass="error" />
					        </td>
					      </tr>
					      <c:if test="${bdMstr.bbsAttrbCode == 'BBSA01'}"> 
					          <tr> 
...
<c:if test="${bdMstr.fileAtchPosblAt == 'Y'}">
<script type="text/javascript">
// 파일 첨부 버튼 초기화 (파일이 없는 경우에만)
<c:if test="${empty result.atchFileId}">
(function() {
    // 약간의 지연으로 DOM 로드 보장
    setTimeout(function() {
        var parentForm = document.board || document.querySelector("form[name='board']");
        if (!parentForm) return;

        // fileListCnt가 아직 설정되지 않았으면 0으로 초기화
        ...
        if (maxFileNumInput && uploader && listContainer) {
            ...
            if (maxFileNum > 0) {
                ...
                if (typeof MultiSelector !== "undefined") {
                    var multi_selector = new MultiSelector(listContainer, maxFileNum);
                    multi_selector.addElement(uploader);
                }
            } else {
                // 첨부 버튼 숨김
